  ...
  return (
    <MobileLayout>
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-green-50 relative overflow-hidden">
        ...
        <div className="px-2.5 py-2 space-y-2.5 relative z-10 pb-20">
          {/* Compact Header */}
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <Button variant="outline" size="sm" className="rounded-full border-emerald-200 hover:bg-emerald-50 h-6 w-6 p-0">
              ...
              <div className="flex gap-1 overflow-x-auto">
                {[
                  ...
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-0.5 px-2 py-0.5 rounded-full text-xs font-medium transition-all duration-300 whitespace-nowrap",
                      filter === item.key
          ...
          <Card className="bg-white/95 backdrop-blur-sm border border-gray-200/50 shadow-xl">
            <CardHeader className="pb-1">
              <CardTitle className="text-sm flex items-center gap-1.5 font-trading">
                ...
                <div className="ml-auto text-xs bg-emerald-100 text-emerald-700 px-1 py-0.5 rounded-full">
              ...
              {isLoading ? (
                <div className="space-y-1 p-1.5">
                  {Array(8).fill(0).map((_, idx) => (
                    <div key={idx} className="animate-pulse flex items-center justify-between p-1.5 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-1.5">
                        <div className="w-6 h-6 bg-gray-200 rounded-lg"></div>
                        <div>
                          <div className="h-2 w-12 bg-gray-200 rounded mb-0.5"></div>
                          <div className="h-1.5 w-6 bg-gray-200 rounded"></div>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="h-2 w-10 bg-gray-200 rounded mb-0.5"></div>
                        <div className="h-1.5 w-5 bg-gray-200 rounded"></div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-0.5 p-1">
                  {getFilteredData().map((crypto, idx) => (
                    <Link
                      key={crypto.id}
                      to={`/coin/${crypto.id}`}
                      className="group flex items-center justify-between p-1.5 rounded-lg hover:bg-gray-50 transition-all duration-300 hover:shadow-md border border-transparent hover:border-gray-200 animate-fade-in"
                      style={{
                    ...
                    >
                      <div className="flex items-center gap-1.5">
                        <div className="relative">
                          <div className="w-6 h-6 bg-gradient-to-r from-gray-100 to-gray-200 rounded-lg flex items-center justify-center group-hover:scale-105 transition-transform">
                            <img 
                              src={crypto.logo} 
                              alt={crypto.name} 
                          ...
                          {idx < 3 && (
                            <div className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-gradient-to-r from-amber-400 to-orange-500 rounded-full flex items-center justify-center">
                      ...
                      <div className="text-right">
                        ...
                        <div className={cn(
                          ...
                          {(crypto.change || 0) >= 0 ? (
                            <ArrowUpRight className="w-1.5 h-1.5 mr-0.5" />
                          ) : (
                            <ArrowDownRight className="w-1.5 h-1.5 mr-0.5" />
                          )}
