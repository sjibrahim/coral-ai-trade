...
import { Check, Wallet, ArrowUp, CreditCard, Clock, Shield, Banknote, TrendingUp, Target, Zap } from "lucide-react";
import { Dialog, DialogContent } from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { createDepositOrder, getGeneralSettings } from "@/services/api";
...
const DepositPage = () => {
  ...
  const [settings, setSettings] = useState<GeneralSettings>({ min_deposit: "100" });
    ...
    if (user) {
      ...
      if (isBankNotSet) {
        toast({
          ...
          description: "Please set up your bank details before making a deposit.",
          variant: "destructive",
        });
        navigate("/bank");
      }
  ...
  const minDeposit = parseInt(settings.min_deposit) || 100;
  const isValidAmount = Number(amount) >= minDeposit && Number(amount) <= availableBalance;
  
  const handleConfirm = async () => {
    if (!isValidAmount) {
      setError(`Please enter a valid amount (minimum ₹${minDeposit} and not exceeding your balance)`);
      return;
    }
    ...
    try {
      ...
      const response = await createDepositOrder(token, Number(amount));
      ...
      if (response.status) {
        ...
        setError(response.msg || "Failed to process deposit");
        toast({
          title: "Deposit Failed",
          description: response.msg || "Failed to process deposit",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  return (
    <MobileLayout showBackButton title="Deposit Funds" hideNavbar>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-center mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-emerald-100">Securely add funds to your wallet</p>
                  </div>
                </div>
              ...
              <CardContent className="p-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-emerald-600 font-bold text-base">₹{amount || '0'}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <DialogContent className="sm:max-w-md bg-gradient-to-br from-emerald-600 to-green-600 border-0 shadow-2xl p-0 overflow-hidden">
          <div className="relative p-6">
            <div className="flex flex-col items-center text-white">
              ...
              >
                Your deposit of ₹{amount} has been successfully processed. Your wallet balance has been updated.
              </motion.p>
              <Button 
